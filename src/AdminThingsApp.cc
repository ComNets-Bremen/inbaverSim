...
#include "AdminThingsApp.h"

Define_Module(AdminThingsApp);

void AdminThingsApp::initialize(int stage)
{
    if (stage == 0) {
            ...
            configurationSettingsStartTime = par("configurationSettingsStartTime");
            configurationParameter = par("configurationParameter").stringValue();
            configurationValue = par("configurationValue");
            ...
            // reminder to send Configuration Settings
            cMessage *configurationSettingsEvent = new cMessage("Configuration Settings Event");
            configurationSettingsEvent->setKind(THINGSAPP_CONFIG_SETTINGS_REMINDER_CODE);
            scheduleAt(simTime() + configurationSettingsStartTime, configurationSettingsEvent);
            ...
            totalContentObjectReceivedCount = registerSignal("apptotalContentObjectReceivedCount");
            totalInterestsSentCount = registerSignal("apptotalInterestsSentCount");
...
void AdminThingsApp::handleMessage(cMessage *msg)
{
    // register app with lower layer (forwarder)
       ...
       // request for a single sensor value download
           ...
           intCount++;
           emit(totalInterestsSentCount, (long) intCount);
       ...
       }//If it is Configuration Settings Reminder Code
       else if (msg->isSelfMessage() && msg->getKind() == THINGSAPP_CONFIG_SETTINGS_REMINDER_CODE){

           //Send Query Interest with Configuration Data
           char completeDataname[64];
           strcpy(completeDataname,"ConfigSet:");
           strcat(completeDataname,configurationParameter.c_str());
           strcat(completeDataname, "-");
           strcat(completeDataname,to_string(configurationValue).c_str());

           InterestMsg* interestMsg = new InterestMsg("Configuration Settings");
           interestMsg->setHopLimit(15);
           interestMsg->setLifetime(simTime() + interestLifetime);
           interestMsg->setPrefixName(requestedSensorNetworkPrefixName.c_str());
           interestMsg->setDataName(completeDataname);
           interestMsg->setVersionName("v01");
           ...
           EV_INFO << simTime() << " Configuration Settings: " << interestMsg->getPrefixName()
                           << " " << interestMsg->getDataName()
                           << " " << interestMsg->getVersionName()
                           << " " << interestMsg->getSegmentNum()
                           << " RNP:" << interestMsg->getReflexiveNamePrefix()
          ...
          // get message arrival gate name
           ...
           if(interestMsg != NULL){
               ...
               coCount++;
               emit(totalContentObjectReceivedCount, (long) coCount);
               ...
               if(string(contentObjMsg->getDataName()).find("ConfigSet") != std::string::npos){

                   EV_INFO << simTime() << " Received Acknowledgement for Configuration Settings " << endl;
