...
void IoTGatewayApp::initialize(int stage)
{
    if (stage == 0) {
        ...
        temperatureLowerLimit = par("temperatureLowerLimit");
        temperatureHigherLimit = par("temperatureHigherLimit");
        lightIntensityLowerLimit = par("lightIntensityLowerLimit");
        lightIntensityUpperLimit = par("lightIntensityUpperLimit");
    ...
    if (msg->isSelfMessage() && msg->getKind() == IOTGATEWAYAPP_APP_REG_REM_EVENT_CODE) {
        ...
        if (interestMsg != NULL) {
            ...
            if (string(interestMsg->getPrefixName()).find(hostedPrefixNameForSensorNet) != string::npos) {
                ...
                //To check if interest is an Actuator Registration Interest
                if(string(interestMsg->getDataName()).find("ActuatorRegRequest") != string::npos) {

                    //Extract the Actuator Name from the Interest
                    string data_name = interestMsg->getDataName();
                    string actuator_name = data_name.substr(data_name.find(":") + 1, data_name.find("-") - data_name.find(":") - 1);
                    string actuatorDataTypes = data_name.substr(data_name.find('-') + 1);

                    EV_INFO << simTime() << " Received Registration Request from a Actuator: "<< actuator_name << endl;

                    // update the actuator name as a actuator in the network
                    int index = -1;
                    for (int i = 0; i < servedActuatorList.size(); i++) {
                        if (string(actuator_name) == servedActuatorList[i]->actuatorName) {
                            index = i;
                            break;
                        }
                    }
                    if (index == -1) {
                        ...
                        ActuatorEntry* actuatorEnry = new ActuatorEntry;
                        actuatorEnry->actuatorName = actuator_name;
                        actuatorEnry->subscriptionStatus = false;
                        actuatorEnry->actuatorDataTypes = actuatorDataTypes;
                        actuatorEnry->lastActuatorState = false;
                        actuatorEnry->faceID = interestMsg->getArrivalFaceID();
                        actuatorEnry->faceType = interestMsg->getArrivalFaceType();

                        servedActuatorList.push_back(actuatorEnry);
                        index = servedActuatorList.size() - 1;
                    }

                    // Acknowledge the Actuator Registration Request  with a content obj
                ...
                //To check if interest is an Actuator Subscription Interest
                if(string(interestMsg->getDataName()).find("ActuatorSubRequest") != string::npos) {

                    EV_INFO << simTime() << " It is a Subscription Request from Actuator" << endl;

                    for (int i = 0; i < servedActuatorList.size(); i++) {
                        if (string(interestMsg->getDataName()).find(servedActuatorList[i]->actuatorName) != std::string::npos) {

                            //Retrieve Subscriber's data
                            string data_name = interestMsg->getDataName();
                            string actuator_name = data_name.substr(data_name.find(":") + 1);

                            // Set the Subscription status of the Actuator to true and also save initial FPT and Prefix name
                            servedActuatorList[i]->subscriptionStatus = true;
                            servedActuatorList[i]->initialFPT = interestMsg->getForwardPendingToken();
                            servedActuatorList[i]->initialReflPrefixName = interestMsg->getReflexiveNamePrefix();
                ...
                //To check if interest from actuator asking for current Actuator Configurations
                if(string(interestMsg->getDataName()).find("QueryActuatorCurrentConfig") != string::npos) {

                    //Search for the current configuration of requested actuator
                   for (int i = 0; i < servedActuatorList.size(); i++){

                       if(string(interestMsg->getDataName()).find(servedActuatorList[i]->actuatorName) != std::string::npos){

                           //Check if Actuator has sent Subscription Interest
                           if(servedActuatorList[i]->subscriptionStatus == true){
                               //Form Data Name for the Content Object to be Sent
                               char completePayload[36];
                               strcpy(completePayload,servedActuatorList[i]->actuatorName.c_str());
                               strcat(completePayload,"/");
                               strcat(completePayload,to_string(servedActuatorList[i]->lastActuatorState).c_str());
                          ...
                          //   contentObjMsg->setExpirytime(simTime() + interestLifetime);
                               contentObjMsg->setHeaderSize(INBAVER_CONTENT_OBJECT_MSG_HEADER_SIZE);
                               contentObjMsg->setPayloadSize(36);
                               contentObjMsg->setTotalNumSegments(1);
                               ...
                               EV_INFO << simTime() << "Actuator has not Subscribed" << endl;
                           }

                       }
                       else{

                           EV_INFO << simTime() << "Actuator has not Registered" << endl;
                       }
                ...
                //If it is a Configuration Setting from Admin
                if(string(interestMsg->getDataName()).find("ConfigSet") != string::npos){

                    //Extract the Configuration Setting from the Interest
                    string data_name = interestMsg->getDataName();
                    string configParameter = data_name.substr(data_name.find(":") + 1, data_name.find("-") - data_name.find(":") - 1);
                    double configValue = stod(data_name.substr(data_name.find('-') + 1));

                    EV_INFO << " Configuration Parameter: " << configParameter
                            << " Configuration Value: "     << configValue
                            << endl;

                    //Update the Corresponding Config Setting
                    if(configParameter == "TempLow"){

                        temperatureLowerLimit = configValue;
                        EV_INFO << " Temperature Lower Limit Updated. New value is " << temperatureLowerLimit << endl;

                    }else if(configParameter == "TempHigh"){

                        temperatureHigherLimit = configValue;
                        EV_INFO << " Temperature Upper Limit Updated. New Value is " << temperatureHigherLimit << endl;

                    }else if(configParameter == "LightLow"){

                        lightIntensityLowerLimit = configValue;
                        EV_INFO << " Light Intensity Lower Limit Updated. New value is " << lightIntensityLowerLimit << endl;

                    }else if(configParameter == "LightHigh"){

                        lightIntensityUpperLimit = configValue;
                        EV_INFO << " Light Intensity Upper Limit Updated. New Value is " << lightIntensityUpperLimit << endl;

                    } else {
                       EV_FATAL << "Invalid Configuration Setting. Check if the Dataname of Configuration Interest was formed correctly. " << endl;
                       // throw cRuntimeError("Check log for details");
                    }

                    //Sending Acknowledgement to the Admin User
                    ContentObjMsg* contentObjMsg = new ContentObjMsg("Content Object");
                    contentObjMsg->setPrefixName(interestMsg->getPrefixName());
                    contentObjMsg->setDataName(interestMsg->getDataName());
                    contentObjMsg->setVersionName(interestMsg->getVersionName());
                ...
                else if(string(interestMsg->getDataName()).find("SubscriptionRequest") != string::npos){
        ...
        if (contentObjMsg != NULL) {
            ...
            if(string(contentObjMsg->getDataName()).find("RegistrationDetails") != string::npos){
                ...
                if (index != -1){
                   ...
                   // CONTROLLER FUNCTIONALITY - To monitor sensor readings and send Reflexive Interest to Actuator (if needed)

                    //Derive the required Temperature,LightIntensity and motion values from the received Payload(From Sensor)
                    string receivedPayload = contentObjMsg->getPayloadAsString();

                    //Check if payload has any data that we are interested in
                    if(string(receivedPayload).find("temperature") != string::npos){

                        // Extract Temperature
                        int temperatureStart = receivedPayload.find("temperature:") + sizeof("temperature:") - 1;
                        int temperatureEnd = receivedPayload.find(":", temperatureStart);

                        if (temperatureStart != string::npos && temperatureEnd != string::npos) {
                            double temperatureValue = stod(receivedPayload.substr(temperatureStart, temperatureEnd - temperatureStart));
                            EV_INFO <<"Retrieved Temperature Value:" << temperatureValue <<endl;

                            //Compare the Temperature value with that of configurations
                            if(temperatureValue < temperatureLowerLimit){

                                //Check which actuators are interested in this data
                                for (int i = 0; i < servedActuatorList.size(); i++) {

                                    if (string(servedActuatorList[i]->actuatorDataTypes).find("temperature") != string::npos) {

                                        //Set the Actuator state to ON
                                        servedActuatorList[i]->lastActuatorState = true;
                                        servedActuatorList[i]->lastActuatorReadingRecordedTime = simTime();
                                        servedActuatorList[i]->lastActuatorReadingsList.push_back(true);

                                        //Send Reflexive Interest to Actuator asking to fetch configurations
                                        //Form Data Name for the Reflexive Interest to be Sent
                                        char completeDataName[64];
                                        strcpy(completeDataName,"FetchConfigurations:");
                                        strcat(completeDataName,servedActuatorList[i]->actuatorName.c_str());

                                        InterestMsg* interestMsgNew = new InterestMsg("Reflexive Interest to Actuator asking to fetch Data");
                                        interestMsgNew->setHopLimit(10);
                                        interestMsgNew->setLifetime(simTime() + interestLifetime);
                                        interestMsgNew->setPrefixName(servedActuatorList[i]->initialReflPrefixName.c_str());
                                        interestMsgNew->setDataName(completeDataName);
                                        interestMsgNew->setVersionName("v01");
                                        interestMsgNew->setSegmentNum(0);
                                        interestMsgNew->setHeaderSize(INBAVER_INTEREST_MSG_HEADER_SIZE);
                                        ...
                                        interestMsgNew->setReflexiveNamePrefix("R");
                                        interestMsgNew->setForwardPendingToken(0);
                                        interestMsgNew->setReversePendingToken(servedActuatorList[i]->initialFPT);

                                        EV_INFO << simTime() << " Sending Reflexive Interest to the Actuator: " << interestMsgNew->getPrefixName()
                                                        << " " << interestMsgNew->getDataName()
                                                        << " " << interestMsgNew->getVersionName()
                                                        << " " << interestMsgNew->getSegmentNum()
                                                        << " RNP:" << interestMsgNew->getReflexiveNamePrefix()
                            ...
                            if(temperatureValue > temperatureHigherLimit){
                                ...
                                for (int i = 0; i < servedActuatorList.size(); i++) {
                                    ...
                                    if (string(servedActuatorList[i]->actuatorDataTypes).find("temperature") != string::npos) {
                                        ...
                                        //Set the Actuator state to ON
                                        servedActuatorList[i]->lastActuatorState = false;
                                        servedActuatorList[i]->lastActuatorReadingRecordedTime = simTime();
                                        servedActuatorList[i]->lastActuatorReadingsList.push_back(false);
                            ...
                            EV_INFO <<"Temperature not found in the string."<< endl;
                        }

                    } else if((receivedPayload).find("lightIntensity") != string::npos){

                        // Extract LightIntensity
                        int lightIntensityStart = receivedPayload.find("lightIntensity:") + sizeof("lightIntensity:") - 1;
                        int lightIntensityEnd = receivedPayload.find(":", lightIntensityStart);

                        if(lightIntensityStart != string::npos && lightIntensityEnd != string::npos) {
                            double lightIntensityValue = stod(receivedPayload.substr(lightIntensityStart, lightIntensityEnd - lightIntensityStart));
                            EV_INFO <<"Retrieved LightIntensity Value:" << lightIntensityValue <<endl;


                            //Compare the Light Intensity value with that of configurations
                            if(lightIntensityValue < lightIntensityLowerLimit){

                                //Check which actuators are interested in this data
                                for (int i = 0; i < servedActuatorList.size(); i++) {

                                    if (string(servedActuatorList[i]->actuatorDataTypes).find("lightIntensity") != string::npos) {
                            ...
                            EV_INFO <<"LightIntensity not found in the string."  <<endl;
                        }

                    } else if((receivedPayload).find("motion") != string::npos){

                        // Extract motion
                        int motionStart = receivedPayload.find("motion:") + sizeof("motion:") - 1;
                        int motionEnd = receivedPayload.find(":", motionStart);

                        if (motionStart != string::npos && motionEnd != string::npos) {
                            int motionValue = stoi(receivedPayload.substr(motionStart, motionEnd - motionStart));
                            EV_INFO <<"Retrieved motion Value:" << motionValue <<endl;
                            //Compare the Motion value with that of configurations
                            if(motionValue == 1){

                                //Check which actuators are interested in this data
                                for (int i = 0; i < servedActuatorList.size(); i++) {

                                    if (string(servedActuatorList[i]->actuatorDataTypes).find("motion") != string::npos) {
                                ...
                                EV_INFO <<"Unexpected motion value " << endl;
                            }

                        } else {
                            EV_INFO <<"motion not found in the string."  <<endl;
                        }

                    }else {

                        EV_INFO <<"Received Payload doesn't have any data that Actuators are interested in"  <<endl;
