...
#include "RFC8569WithRFForwarder.h"

Define_Module(RFC8569WithRFForwarder);

void RFC8569WithRFForwarder::initialize(int stage)
{
    if (stage == 0) {
      ...
      //  totalBytesReceivedAndSent = 0;
        ...
        statGenReminderEvent->setKind(RFC8569WITHRFFWD_STAT_GEN_REM_EVENT_CODE);
...
void RFC8569WithRFForwarder::handleMessage(cMessage *msg)
{
    cGate *arrivalGate;
    char gateName[32];
    ...
    if (msg->isSelfMessage()) {
        ...
        if (msg->getKind() == RFC8569WITHRFFWD_STAT_GEN_REM_EVENT_CODE) {
...
void RFC8569WithRFForwarder::processApplicationRegistration(AppRegistrationMsg *appRegMsg)
{
...
void RFC8569WithRFForwarder::processPrefixRegistration(PrefixRegistrationMsg *prefixRegMsg)
{
...
void RFC8569WithRFForwarder::processTransportRegistration(TransportRegistrationMsg *transportRegMsg)
{
...
void RFC8569WithRFForwarder::processInterest(InterestMsg *interestMsg)
{
...
//        dumpPIT();
       ...
       while (iteratorFaceEntry != fibEntry->forwardedFaces.end()) {
           faceEntry = *iteratorFaceEntry;
           EV_INFO << simTime() << "Retreived Faces: " << faceEntry->faceID <<endl;
...
void RFC8569WithRFForwarder::processContentObj(ContentObjMsg *contentObjMsg)
{
...
void RFC8569WithRFForwarder::processInterestRtn(InterestRtnMsg *interestRtnMsg)
{
    // get arrival gate details
...
FaceEntry *RFC8569WithRFForwarder::getFaceEntryFromInputGateName(string inputGateName, int gateIndex)
{
    FaceEntry *faceEntry = NULL;
...
FaceEntry *RFC8569WithRFForwarder::getFaceEntryFromFaceID(long faceID)
{
    FaceEntry *faceEntry = NULL;
...
CSEntry *RFC8569WithRFForwarder::getCSEntry(string prefixName, string dataName, string versionName, int segmentNum)
{
...
PITEntry *RFC8569WithRFForwarder::getPITEntry(string prefixName, string dataName, string versionName, int segmentNum)
{
...
PITEntry *RFC8569WithRFForwarder::getPITEntryUsingRPT(int rpt)
{
    list<PITEntry*>::iterator iteratorPITEntry = pit.begin();
    while (iteratorPITEntry != pit.end()) {
        PITEntry *pitEntry = *iteratorPITEntry;
...
void RFC8569WithRFForwarder::updateCSEntry()
{
...
FIBEntry *RFC8569WithRFForwarder::updateFIB(string prefixName, FaceEntry *faceEntry)
{
...
void RFC8569WithRFForwarder::updatePITEntry()
{
    //check if CS has any records
    if(pit.empty()== false){
...
FIBEntry *RFC8569WithRFForwarder::longestPrefixMatchingInFIB(string prefixName)
{
...
void RFC8569WithRFForwarder::finish()
{
    // dump info
//    dumpFaces();
//    dumpFIB();
...
void RFC8569WithRFForwarder::dumpFIB()
{
    EV_INFO << simTime() << "==FIB===\n";
    list<FIBEntry*>::iterator iteratorFIBEntry = fib.begin();
    while (iteratorFIBEntry != fib.end()) {
...
void RFC8569WithRFForwarder::dumpFaces()
{
    FaceEntry *faceEntry = NULL;
...
void RFC8569WithRFForwarder::dumpCS()
{
    EV_INFO << simTime() << "==CS===\n";
...
void RFC8569WithRFForwarder::dumpPIT()
{
    EV_INFO << simTime() << "==PIT===\n";
    list<PITEntry*>::iterator iteratorPITEntry = pit.begin();
    while (iteratorPITEntry != pit.end()) {
